...
# Cours — Comprendre les outils IA modernes : Streamlit, FastAPI, Pydantic, JSON, HTTP, H2O.ai, MLOps, Docker, VM et conteneurs
...
| 2  | [Pourquoi autant d’outils dans un projet IA ?](#section-2)        |
| 3  | [Vue globale d’un projet IA moderne](#section-3)                  |
| 4  | [Python : pourquoi ce langage est utilisé en IA ?](#section-4)    |
| 5  | [Environnement virtuel Python : pourquoi ?](#section-5)           |
| 6  | [Bibliothèques, dépendances et conflits de versions](#section-6)  |
| 7  | [Streamlit : c’est quoi ?](#section-7)                            |
| 8  | [Pourquoi utiliser Streamlit ?](#section-8)                       |
| 9  | [FastAPI : c’est quoi ?](#section-9)                              |
| 10 | [FastAPI ou FastPi : attention à la confusion](#section-10)       |
| 11 | [Streamlit vs FastAPI](#section-11)                               |
...
| 21 | [Swagger et OpenAPI : documentation automatique](#section-21)     |
...
Un projet d’intelligence artificielle ne se limite pas à entraîner un modèle dans un notebook. Dans un vrai projet, il faut aussi penser à la manière dont ce modèle sera utilisé, partagé, testé, déployé, surveillé et maintenu.

Un modèle IA peut donner de bons résultats dans un fichier Python, mais cela ne signifie pas qu’il est prêt pour une entreprise. Pour qu’un modèle devienne utile, il doit être intégré dans un système.
...
> [!TIP]
> Le modèle IA est seulement une partie du système. En entreprise, la vraie valeur vient de la capacité à transformer ce modèle en service fiable, utilisable et maintenable.

> [!IMPORTANT]
> Un notebook est utile pour expérimenter. Une API, un conteneur, une interface et une stratégie MLOps sont nécessaires pour construire un système utilisable dans un contexte réel.
...
<details>
<summary><strong>2 — Pourquoi autant d’outils dans un projet IA ?</strong></summary>

<br/>

Un projet IA moderne contient plusieurs responsabilités. Chaque outil répond à un problème précis.
...
### Diagramme Mermaid — Rôle des outils
...
> [!TIP]
> Chaque outil a un rôle. Un bon projet IA ne consiste pas à empiler des outils au hasard, mais à choisir les bons outils pour répondre aux bons besoins.

> [!CAUTION]
> Danger pédagogique : si les étudiants ne comprennent pas le rôle de chaque outil, ils risquent d’utiliser Streamlit comme backend, FastAPI comme interface, Docker comme machine virtuelle, ou MLOps comme simple mot marketing.
...
<details>
<summary><strong>3 — Vue globale d’un projet IA moderne</strong></summary>
...
> [!IMPORTANT]
> Dans un projet professionnel, il faut penser dès le départ à la question suivante : comment le modèle sera-t-il utilisé par un humain ou par une autre application ?
...
<details>
<summary><strong>4 — Python : pourquoi ce langage est utilisé en IA ?</strong></summary>

<br/>

Python est l’un des langages les plus utilisés en intelligence artificielle parce qu’il est simple à lire, riche en bibliothèques et très utilisé dans les communautés data science, machine learning, deep learning et automatisation.
...
### Diagramme Mermaid — Python au centre
...
> [!TIP]
> Python est populaire en IA parce qu’il permet de passer rapidement de l’idée au prototype.
...
Si les deux projets utilisent le même environnement global, une mise à jour pour le projet 2 peut casser le projet 1.
...
### Diagramme Mermaid — Environnement global vs environnements virtuels
...
> [!CAUTION]
> Danger : installer toutes les bibliothèques dans Python global peut donner l’impression de gagner du temps au début, mais cela crée souvent des problèmes plus tard.
...
<details>
<summary><strong>6 — Bibliothèques, dépendances et conflits de versions</strong></summary>
...
Chez l’étudiant, ça ne marche pas.
Sur le serveur, ça ne marche plus.
```

### Diagramme Mermaid — Problème de dépendances
...
* un tableau de bord ;
* une démo pour un client ou un professeur.
...
### Diagramme Mermaid — Rôle de Streamlit
...
> [!IMPORTANT]
> Streamlit est surtout une interface. Il ne remplace pas forcément une API professionnelle ou un backend complet.
...
Dans un cours, Streamlit est très utile parce que les étudiants voient rapidement le résultat du modèle.
...
| Pédagogie     | Les étudiants comprennent mieux le résultat |
...
### Diagramme Mermaid — Streamlit dans un prototype
...
> [!TIP]
> Streamlit est excellent pour la phase “prototype”. Il permet de prouver rapidement qu’une idée fonctionne.

> [!WARNING]
> Danger : ne pas croire qu’un prototype Streamlit est automatiquement une application de production prête pour des milliers d’utilisateurs.
...
### Diagramme Mermaid — FastAPI comme backend
...
> [!IMPORTANT]
> Dans un projet IA, FastAPI est souvent le pont entre le modèle et le monde extérieur.
...
<details>
<summary><strong>10 — FastAPI ou FastPi : attention à la confusion</strong></summary>
...
### Diagramme Mermaid — Différence FastAPI / Raspberry Pi
...
> Raspberry Pi répond à la question : “Sur quelle petite machine puis-je exécuter un programme ?”
...
<details>
<summary><strong>11 — Streamlit vs FastAPI</strong></summary>
...
| Usage courant         | Démo, dashboard, prototype     | Backend, service, intégration |
| Exemple               | Formulaire de prédiction       | Route `/predict`              |
| Documentation API     | Pas le rôle principal          | Swagger automatique           |
| Production            | Possible mais limité selon cas | Très adapté aux services API  |

### Diagramme Mermaid — Les deux ensemble
...
> [!TIP]
> Dans un cours, faire travailler Streamlit et FastAPI ensemble aide les étudiants à comprendre la séparation frontend / backend.

> [!WARNING]
> Danger : si toute la logique critique est directement dans Streamlit, le projet peut devenir difficile à maintenir, tester et sécuriser.
...
Quand tu utilises une application météo, ton téléphone ne mesure pas lui-même la température. Il demande l’information à un serveur météo.
...
### Diagramme Mermaid — Communication avec une API
...
### Diagramme Mermaid — Requête et réponse
...
### Diagramme Mermaid — Routes d’une API IA
...
> [!WARNING]
> Danger : une API avec des routes mal nommées devient difficile à utiliser, même si le code fonctionne.
...
> [!WARNING]
> Danger : il ne faut pas envoyer un mot de passe ou un secret dans l’URL avec GET, car l’URL peut être visible dans les historiques et les logs.
...
> [!CAUTION]
> Danger : DELETE doit être protégé par des permissions. Tout le monde ne doit pas avoir le droit de supprimer des ressources.
...
### Diagramme Mermaid — Choisir la bonne méthode
...
Ces codes sont moins visibles pour les débutants, car ils sont souvent gérés automatiquement.
...
| Code | Nom        | Exemple                                 |
| ---- | ---------- | --------------------------------------- |
| 200  | OK         | La liste des étudiants est retournée    |
| 201  | Created    | Un nouvel étudiant est créé             |
...
> [!CAUTION]
> Danger : si une API retourne souvent des erreurs 500, il faut consulter les logs du serveur. Le problème vient rarement de l’utilisateur final.

### Diagramme Mermaid — Diagnostic par code HTTP
...
### Diagramme Mermaid — JSON dans une API
...
> [!WARNING]
> Danger : un JSON mal formé peut provoquer une erreur 400. Un JSON bien formé mais incorrect par rapport au modèle attendu peut provoquer une erreur 422.
...
1. Quel est l’identifiant de l’étudiant ?
2. Quel est le nom de l’étudiant ?
...
> [!TIP]
> Les exercices JSON sont très utiles avant de faire FastAPI, parce que les API modernes échangent souvent des données en JSON.
...
### Diagramme Mermaid — Validation avec Pydantic
...
### Diagramme Mermaid — Avant / après Pydantic
...
> [!CAUTION]
> Danger : si on ne valide pas les entrées, le modèle peut recevoir des données absurdes et retourner une prédiction qui semble correcte mais qui est en réalité fausse.
...
<details>
<summary><strong>21 — Swagger et OpenAPI : documentation automatique</strong></summary>
...
Swagger permet de tester l’API sans créer une application cliente complète.

Un étudiant peut ouvrir la documentation, cliquer sur une route, entrer un JSON et observer la réponse.

### Diagramme Mermaid — Swagger dans le cycle de test

```mermaid
flowchart TD
    A[Développeur ou étudiant] --> B[Swagger UI]
...
> [!TIP]
> Swagger est excellent pour apprendre les API parce qu’il rend visibles les routes, les méthodes, les modèles et les réponses.

> [!IMPORTANT]
> Une bonne documentation API réduit les erreurs de communication entre les développeurs frontend, backend, data science et DevOps.

> [!WARNING]
> Danger : en production, il faut réfléchir à l’exposition de la documentation Swagger. Dans certains contextes, il ne faut pas exposer publiquement toute la documentation de l’API.
...
### Diagramme Mermaid — Principe AutoML
...
> [!WARNING]
> Danger : AutoML ne remplace pas la compréhension des données. Un outil peut proposer un bon score, mais le professeur, l’ingénieur ou l’analyste doit comprendre pourquoi le modèle est acceptable ou non.
...
### Diagramme Mermaid — AutoML avec contrôle humain
...
> [!CAUTION]
> Danger : choisir automatiquement le modèle avec le meilleur score sans comprendre la métrique, les données et le contexte peut mener à une mauvaise décision.
...
> [!TIP]
> MLOps est ce qui permet de passer d’un modèle expérimental à un modèle exploitable en production.
...
### Diagramme Mermaid — Risque sans MLOps
...
> [!CAUTION]
> Danger : un modèle peut continuer à répondre même s’il est devenu mauvais. C’est plus dangereux qu’une application qui plante, car l’erreur peut être silencieuse.
...
### Diagramme Mermaid — Machine physique et VM
...
Un étudiant doit apprendre à installer un serveur Linux.

Au lieu de modifier son ordinateur personnel, il crée une VM.

S’il casse la configuration, il peut supprimer la VM et recommencer.
...
### Diagramme Mermaid — Rôle de l’hyperviseur
...
### Diagramme Mermaid — VM vs conteneur
...
> [!CAUTION]
> Danger : un conteneur n’est pas une mini-machine virtuelle complète. Il ne faut pas confondre les deux.
...
### Diagramme Mermaid — Cycle Docker
...
### Diagramme Mermaid — Image, registre et déploiement
...
### Diagramme Mermaid — Projet avec plusieurs conteneurs
...
| Enseigner proprement           | Structure claire pour les étudiants  |

> [!TIP]
> Docker Compose est très utile pour les laboratoires pédagogiques qui combinent FastAPI, Streamlit, MLflow et une base de données.
...
L’objectif est de faire comprendre aux étudiants les rôles de chaque outil sans rendre le projet trop complexe.

### Architecture pédagogique

```mermaid
flowchart TD
    A[Étudiant] --> B[Streamlit]
    B --> C[FastAPI]
    C --> D[Pydantic]
...
> [!WARNING]
> Danger : plus l’architecture est professionnelle, plus elle nécessite de discipline. Il faut documenter, tester, sécuriser et surveiller.

> [!TIP]
> Le rôle du cours est de commencer simple, puis de montrer progressivement comment les briques se combinent dans une architecture plus réaliste.
...
Un étudiant doit apprendre à lire les codes HTTP.
...
> [!CAUTION]
> Danger : une architecture qui fonctionne une fois en local n’est pas forcément une architecture fiable.
...
| --------------------------------------------------------- | ---------------- |
| Quel outil peut servir à créer une interface rapide ?     | Streamlit        |
| Quel outil peut servir à créer une API ?                  | FastAPI          |
| Quel outil peut valider les données JSON ?                | Pydantic         |
| Quel format peut être utilisé pour échanger les données ? | JSON             |
| Quel outil peut aider à conteneuriser ?                   | Docker           |
| Quel concept permet de suivre le modèle en production ?   | MLOps            |
| Quel outil peut automatiser des essais ML ?               | H2O.ai           |
...
> [!TIP]
> Cette activité permet de vérifier que les étudiants comprennent les outils, mais aussi les liens entre les outils.
...
Swagger permet de documenter et tester l’API.

H2O.ai et AutoML permettent d’accélérer certains essais machine learning.
...
> [!CAUTION]
> Danger final : un modèle sans API, sans validation, sans déploiement, sans logs et sans suivi reste une expérience, pas une solution professionnelle.
