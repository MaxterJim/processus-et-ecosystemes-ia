# =============================================================================
#  01-setup-venv-py312.ps1
#  Script d'installation automatique pour DÉBUTANTS sur Windows
#  >>> VERSION POUR PYTHON 3.12 (version par défaut, recommandée) <<<
#  -----------------------------------------------------------------------------
#  Ce script :
#    1. Vérifie que Python 3.12 est installé
#    2. Crée (si absent) un environnement virtuel (venv) nommé "myfastapienv"
#    3. Active l'environnement virtuel
#    4. Met à jour pip
#    5. Installe uniquement les paquets MANQUANTS
#       (FastAPI, Uvicorn, Streamlit, Pandas, NumPy, etc.)
...
#  lancez-le en le "dot-sourçant" (notez le point et l'espace au début) :
#     . .\scripts\01-setup-venv-py312.ps1
#
...
# --- Paramètres spécifiques à cette version ----------------------------------
$PythonVersion = "3.12"
$VenvName      = "myfastapienv"

# --- Petite fonction utilitaire pour écrire en couleurs ----------------------
...
# --- ÉTAPE 0 : Afficher l'en-tête --------------------------------------------
Clear-Host
Write-Host ""
Write-Host "  ╔══════════════════════════════════════════════════════════╗" -ForegroundColor Magenta
Write-Host "  ║  Installation automatique — Projet FastAPI / Streamlit   ║" -ForegroundColor Magenta
Write-Host "  ║  Version Python : $PythonVersion (version par défaut recommandée)  ║" -ForegroundColor Magenta
...
try {
    ...
    Write-Host "  1. Allez sur https://www.python.org/downloads/release/python-3120/"
...
# --- ÉTAPE 2 : Créer le venv (uniquement s'il n'existe pas) -----------------
...
# Liste organisée par catégorie pour clarté
$packages = @(
    # --- Backend FastAPI ---
    "fastapi",
    "uvicorn[standard]",
    ...
    # --- Requêtes HTTP / API clients ---
    "requests",
    "httpx",

    # --- Data science / Machine Learning (niveau débutant) ---
    "pandas",
    "numpy",
    "matplotlib",
    "seaborn",
...
# Récupère UNE SEULE fois la liste des paquets déjà installés
# (beaucoup plus rapide que d'appeler pip show N fois)
...
foreach ($pkg in $packages) {
    $index++
    # Extraire le nom de base (sans "[extras]" ni "==version")
...
Write-OK "Fichier 'requirements.txt' généré à la racine du projet."
...
Write-Host ""
Write-Host "  Pour lancer un backend FastAPI (exemple) :" -ForegroundColor Cyan
Write-Host "    uvicorn backend:app --reload"
Write-Host ""
Write-Host "  Pour lancer un frontend Streamlit (exemple) :" -ForegroundColor Cyan
Write-Host "    streamlit run frontend.py"
Write-Host ""
Write-Host "  Pour RÉACTIVER le venv lors des prochaines sessions :" -ForegroundColor Cyan
Write-Host "    .\$VenvName\Scripts\Activate.ps1"
Write-Host ""
Write-Host "  Pour DÉSACTIVER le venv :" -ForegroundColor Cyan
Write-Host "    deactivate"
Write-Host ""

# Rappel important si le script n'a pas été dot-sourcé
if ($MyInvocation.InvocationName -ne ".") {
    ...
    Write-Host ""
    Write-Host "  Ou, pour éviter cela à l'avenir, lancez le script avec un" -ForegroundColor Yellow
    Write-Host "  point devant (dot-sourcing) :" -ForegroundColor Yellow
    Write-Host "    . .\scripts\01-setup-venv-py312.ps1" -ForegroundColor Yellow
    Write-Host ""
}
