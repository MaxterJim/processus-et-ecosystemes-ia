# =============================================================================
#  06-setup-venv-py39.ps1
#  Script d'installation automatique pour DÉBUTANTS sur Windows
#  >>> VERSION POUR PYTHON 3.9 (ancienne version — support limité) <<<
#  -----------------------------------------------------------------------------
#  Ce script :
#    1. Vérifie que Python 3.9 est installé
#    2. Crée (si absent) un environnement virtuel "myfastapienv-py39"
#    3. Active l'environnement virtuel
#    4. Met à jour pip
#    5. Installe uniquement les paquets MANQUANTS
#    6. Génère un fichier requirements.txt
#
#  ATTENTION : Python 3.9 est une version ancienne. Certaines bibliothèques
#  récentes peuvent ne plus la supporter. Préférez 3.11 ou 3.12 si possible.
#
#  UTILISATION (depuis la racine du projet dans PowerShell) :
#     .\scripts\06-setup-venv-py39.ps1
#
...
try {
    ...
    Write-Host "  1. Allez sur https://www.python.org/downloads/release/python-390/"
...
foreach ($pkg in $packages) {
    ...
    if ($installedNames.ContainsKey($pkgKey)) {
        ...
        if ($LASTEXITCODE -ne 0) {
            ...
            Write-Info "Note : certaines bibliothèques récentes ne supportent plus Python 3.9."
            Write-Info "Essayez une version plus récente (3.11 ou 3.12)."
            exit 1
        }
        $installed++
    }
...
pip freeze | Out-File -Encoding utf8 "requirements-py39.txt"
Write-OK "Fichier 'requirements-py39.txt' généré à la racine du projet."
...
if ($MyInvocation.InvocationName -ne ".") {
    ...
    Write-Host "    . .\scripts\06-setup-venv-py39.ps1" -ForegroundColor Yellow
    Write-Host ""
}
