# =============================================================================
#  02-setup-venv-py314.ps1
#  Script d'installation automatique pour DÉBUTANTS sur Windows
#  >>> VERSION POUR PYTHON 3.14 <<<
#  -----------------------------------------------------------------------------
#  Ce script :
#    1. Vérifie que Python 3.14 est installé
#    2. Crée (si absent) un environnement virtuel "myfastapienv-py314"
...
#     .\scripts\02-setup-venv-py314.ps1
#
...
try {
    ...
    Write-Host "  1. Allez sur https://www.python.org/downloads/release/python-3140/"
    Write-Host "     (ou https://www.python.org/downloads/ si la version exacte n'est pas listée)"
...
foreach ($pkg in $packages) {
    ...
    if ($installedNames.ContainsKey($pkgKey)) {
        ...
        if ($LASTEXITCODE -ne 0) {
            ...
            Write-Info "Note : certaines bibliothèques peuvent ne pas encore supporter Python 3.14."
            Write-Info "Essayez alors une version plus stable (3.12 ou 3.11)."
            exit 1
        }
        $installed++
    }
...
pip freeze | Out-File -Encoding utf8 "requirements-py314.txt"
Write-OK "Fichier 'requirements-py314.txt' généré à la racine du projet."
...
if ($MyInvocation.InvocationName -ne ".") {
    ...
    Write-Host "    . .\scripts\02-setup-venv-py314.ps1" -ForegroundColor Yellow
    Write-Host ""
}
